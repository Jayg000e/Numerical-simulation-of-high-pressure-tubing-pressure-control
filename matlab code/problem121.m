function i=problem121(pumpt)    %通过该程序测试单向阀不同开启时长pumpt使压力从100MPa升至150MPa，并绘制高压油管压力随时间变化的图像
...
for i=1:1000000     %将整个时间段划分为1000000个区间，每个区间为0.01ms，利用数值积分方法计算结果，每个出油周期为100ms，即10000次循环为一出油周期
    if ro(i)>0.8679     %如果高压油管内密度大于0.8679（即高压油管内压力大于150MPa），则跳出循环，终止运行
        break;
    end
    t(i)=i/100;                      %换算使t(i)单位为ms
    t1(i)=rem(i,100*100)/100;        %计算当前时间在当前周期的时间（周期开始为0时间）
    t2(i)=rem(i,100*(pumpt+10))/100;
...
%disp(i/100);
title('每次单向阀开启时长为0.700ms时压力随时间的变化');
xlabel('时间/ms');
ylabel('压力/MPa');
