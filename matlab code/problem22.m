function problem22(omega) %以模型第二题假设二为基础，根据输入的不同角速度计算最终对压力的影响并画出图像，从而确定合理的角速度范围

%如果需要更改模型运作的总时长，请更改下面的变量totali
periodi=ceil(2*pi/omega*100);   %凸轮以角速度omega转动一周的周期periodi，单位0.01ms
totali=10000;         %测试模型运作的总时间totali，单位0.01ms
...
for i=1:totali   %将整个时间段划分为totali个区间，每个区间为0.01ms，利用数值积分方法计算结果，每个出油周期为100ms，即10000次循环为一出油周期
    remi=rem(i,10000);
    if remi<=periodi&&remi~=0   %如果当前时间凸轮仍在运转
        ...
        if remi==1&&i~=1   %如果当前时间刚好进入一个新的周期，则重新初始化柱塞腔内油量质量、柱塞腔内油量密度、高压油管内油量质量与凸轮角度
            mpump(i)=mpump(1);
            ropump(i)=roF(0.5);
            mpipe(i)=mpipe(i-1);
            fi(i)=0;
        else
            fi(i)=omega*t(remi);   %否则根据当前周期时间t(i)计算当前角速度f(i)
        end
        ...
        if pumpout(i-1)~=0   %如果上一个时间i仍在出油，则判断当前时间出油量
            if remi<=246&&remi~=0
               ti=rem(i,10000);
            else 
               ti=1;
            ...
            ppipe(i)=Fro(ropipe(i));
            pumpin(i)=0;
            pumpout(i)=C*qiuS(hneedle(ti))*sqrt(2*ppipe(i)/ropipe(i))*ropipe(i)*0.01;
            mpipe(i+1)=mpipe(i)-pumpout(i);
            mpump(i+1)=mpump(i);
        else    %否则当前时间高压油管进油量与出油量都为0，与上一时间i状态相同
            t(i)=i/100;
            fi(i)=0;
            hpump(i)=hpump(i-1);
            vpump(i)=vpump(i-1);
...
title('角速度为3.35rad/ms时1s内压力随时间的变化');
xlabel('时间/ms');
ylabel('压力/MPa');