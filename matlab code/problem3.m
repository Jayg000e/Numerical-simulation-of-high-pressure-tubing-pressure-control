function problem3(omega,start,foot,offTime)    %以模型三为基础，输入的参数分别为角速度omega、喷油嘴开始工作的时间start、两个喷油嘴开始工作的间隔foot、减压阀关闭的时间offTime（减压阀在喷油结束后立刻开启），计算最终对压力的影响并画出图像，从而确定合理的参数范围

%如果需要更改模型运作的总时长，请更改下面的变量totali
periodi=ceil(2*pi/omega*100);   %凸轮以角速度omega转动一周的周期periodi，单位0.01ms
totali=5000000;         %测试模型运作的总时间totali，单位0.01ms
...
for i=1:totali   %将整个时间段划分为totali个区间，每个区间为0.01ms，利用数值积分方法计算结果，每个出油周期为100ms（每个出油周期两个喷油嘴分别出一次油），即10000次循环为一出油周期
    remi=rem(i,10000);
    if remi<=246+start&&remi>start&&rem(i,10000)~=0
        ti=rem(i-start,10000);    %计算每个出油周期中喷油嘴针阀运动的当前时间ti
        open=0;    %open为当前时间减压阀状态，关闭为0，开启为1，当前时间正在出油所以处于关闭状态
    else 
        if  remi>start+foot&&remi<=246+foot+start&&rem(i,10000)~=0
            ti=rem(i-foot-start,10000);    %计算每个出油周期中喷油嘴针阀运动的当前时间ti
            open=0;
        else
            ti=1;     %如果当前时间喷油嘴不在运作，则令ti=1，即当前针阀高度为0
            if rem(i,5000)>offTime
                open=0;
            else
                open=1;     %当前时间减压阀处于开启状态
            end
        end
    end
    ...
    mpipe(i+1)=mpipe(i)+pumpin(i)-pumpout(i)-open*C*A1*sqrt(2*ppipe(i)/ropipe(i))*ropipe(i)*0.01;   %递推下一i时间内高压油管内油量质量（减去减压阀出油质量）及柱塞腔内油量质量
...
title('50s内角速度为0.4pi，减压阀每周期关闭6.25ms时压力随时间的变化');
xlabel('时间/ms');
ylabel('压力/MPa');